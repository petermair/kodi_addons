...
ADDON = xbmcaddon.Addon(id='plugin.video.plex-tv')
...
def setGlobalProperty(key, val):
    xbmcgui.Window(10000).setProperty(
        'plugin.video.plex-tv.{0}'.format(key), val)


def setGlobalBoolProperty(key, boolean):
    xbmcgui.Window(10000).setProperty(
        'plugin.video.plex-tv.{0}'.format(key), boolean and '1' or '')


def getGlobalProperty(key):
    return xbmc.getInfoLabel(
        'Window(10000).Property(plugin.video.plex-tv.{0})'.format(key))
...
def dialog(typus, *args, **kwargs):
    ...
    if 'icon' in kwargs:
        types = {
            '{plex}': 'special://home/addons/plugin.video.plex-tv/icon.png',
