...
class TvShowMixin(object):
    ...
    def remove(self, plex_id, plex_type=None):
        ...
        if db_item['plex_type'] == v.PLEX_TYPE_EPISODE:
            ...
            # Show verification
            if (not self.plexdb.show_has_seasons(db_item['tvshow_id']) and
                    not self.plexdb.show_has_episodes(db_item['tvshow_id'])):
                # No seasons for show left - so delete entire show
                self.remove_show(db_item['grandparent_id'])
                self.plexdb.remove(db_item['tvshow_id'], v.PLEX_TYPE_SHOW)
        # SEASON #####
...
class Show(TvShowMixin, ItemBase):
    ...
    def add_update(self, xml, section_name=None, section_id=None,
        ...
        if not show:
            update_item = False
            kodi_id = self.kodidb.new_tvshow_id()
        else:
            update_item = True
...
class Season(TvShowMixin, ItemBase):
    def add_update(self, xml, section_name=None, section_id=None,
        ...
        if not season:
            ...
            update_item = True
        tvshow_id = api.parent_id()
        show = self.plexdb.show(tvshow_id)
        if not show:
            LOG.warn('Parent TV show %s not found in DB, adding it', tvshow_id)
            show_xml = PF.GetPlexMetadata(tvshow_id)
            try:
                show_xml[0].attrib
            except (TypeError, IndexError, AttributeError):
                LOG.error("Parent tvshow %s xml download failed", tvshow_id)
                return False
            Show(self.last_sync,
                 plexdb=self.plexdb,
            ...
            show = self.plexdb.show(tvshow_id)
            if not show:
                LOG.error('Still could not find parent tv show %s', tvshow_id)
                return
...
class Episode(TvShowMixin, ItemBase):
    def add_update(self, xml, section_name=None, section_id=None,
        ...
        show = self.plexdb.show(api.tvshow_id())
        if not show:
            LOG.warn('Grandparent TV show %s not found in DB, adding it', api.tvshow_id())
            show_xml = PF.GetPlexMetadata(api.tvshow_id())
            try:
                show_xml[0].attrib
            except (TypeError, IndexError, AttributeError):
                LOG.error("Grandparent tvshow %s xml download failed", api.tvshow_id())
                return False
            Show(self.last_sync,
                 plexdb=self.plexdb,
                 kodidb=self.kodidb).add_update(show_xml[0],
            ...
            show = self.plexdb.show(api.tvshow_id())
            if not show:
                LOG.error('Still could not find grandparent tv show %s', api.tvshow_id())
                return
